...
Status: **CLOSED（機制已消除，不再追因）** —— 使用者裁示 2026-08-21。
  判準 6（資料完整）、7（還原演練）已達成。**判準 8 以「機制消除」而非「尖峰重現」結案**，理由見下方
  「⬆ 2026-08-21 結案」節。⚠ 本 BR **未**證明歷史那組 30.7s 尖峰的成因，
  它至今 **UNDECIDED**——結案的依據是兩個已知候選均已被消除或排除，不是找到了答案。
...
## 觀察期紀錄（dispatcher ses_fe7b5cbadffeSlxj0dv1Z740O4，2026-08-21）

### 一格時序事實，先前沒有人比對過

`BR-20260820_124500` §三把「後端 20-27s」列為它的殘留，**那組量測是 `c663041`（08-20 13:52）做的**。
而 DB 搬離 NFS 是 `dec5b44`（08-20 **17:58**）——**晚四小時**。

```
git log -1 --format='%h %ad %s' --date=format:'%m-%d %H:%M' dec5b44
  → dec5b44 08-20 17:58 fix(infra): SQLite DB 從 NAS(NFS) 搬到本地 ext4
git log -1 --format='%h %ad' --date=format:'%m-%d %H:%M' -- issues/BR-...124500....md
  → c663041 08-20 13:52
```

**所以 BR-124500 §三那個殘留的事實基礎已經被改變過，而它從未重量。**
兩張 BR 的觀察對象是同一個（同一支 API、同樣忽快忽慢、host 與 browser 同步慢），
應合併判定——這正是 §三「後續應查的方向」第 3 點預告的事。

### idle 量測（**不能銷帳，理由在下面**）

```
/api/collections 20 連發（2026-08-21，DB 已在本地 ext4）
  N=20  bad=0  min=0.004  median=0.004  max=0.006
  over_2s=0   over_20s=0
當下負載：JOBS=[]（無 crawler job）；openshelf-app CPU=0.38%
DB 落點：./data/db/openshelf.sqlite  110MB
  df -T data/db → /dev/sde ext4        ← 已非 NFS
```

**這組數字對判準 8 的貢獻是 0。** 判準 8 問的是「尖峰是否消失」，而尖峰是**偶發、有負載時**
發生的。idle 下量 20 次全綠，與本 BR header 自己寫的陷阱是同一句話：

> **「我沒量到」與「它不存在」共用同一個輸出。**

把這組 4ms 當成銷帳，就是拿一個沒有鑑別力的檢查下結論——本 BR 已經因為缺控制組判錯兩次
（`ls` 無 `-wal`、`total > timeout` + 200），這會是第三次。**故記錄為觀察資料，不改 Status。**

### 已派工（handler `ses_fdf8fc2c4ffeHyJI2iGo3sB5b8`，2026-08-21）

目標：**在有負載的條件下**判定尖峰是否仍存在，解本 BR 判準 8 的鎖。
派工單明列三格 handler 最可能該推翻 dispatcher 的：

1. **「有負載」的定義是抄歷史觀察來的，不是從機制推導的。** 真正的負載因子可能是併發連線數，
   或**單一 SQLite 連線無 pool 導致長查詢排隊**（即 §三「後續應查的方向」第 2 點）。
2. **也許不該用「重現尖峰」當判準。** 偶發事件的「重現失敗」永遠可解釋成運氣不好——
   那正是本案卡住一輪的原因。改量**分布尾端**（p99 / max over N）或**直接量鎖等待**
   （SQLite busy 次數、連線持有時間），把偶發事件變成連續指標，可能才是正解。
3. **`--reload` 也許才是主因，而它只在開發環境存在。** 前一輪的決定性實驗
   （`touch app/db/search.py` 觸發 reload）量到 **2.0s**，比觀察值小一個數量級，
   但那次是在單一 host、無並發、無 crawler 下做的。

派工單的硬條件：**任何「尖峰已消失」的宣稱必須附一個能證明該量測在尖峰存在時真的會抓到它的
控制組**（例如人為製造一次已知長延遲，證明探針量得到）。沒有這個控制組，「已消失」是
undecidable 不是結論——三種結論（仍存在 / 已消失 / 仍 undecidable）任一都是合法交件。

---

## ⬆ 2026-08-21 結案（使用者裁示）—— 以「機制已消除」結，**不是**以「找到成因」結

⭕ **先讀這一句：歷史那組 30.7s 尖峰的成因至今 UNDECIDED。**
本節不是在宣布找到了答案，而是在宣布**兩個已知候選均已不可能再發生**，
因此繼續追因的預期收益不再覆蓋成本。這兩件事不同，不得混為一談。

### 兩個候選的下場（各自有獨立證據）

| 候選 | 下場 | 證據 |
|---|---|---|
| **鎖爭用（原候選 1）** | **排除** | WAL 下負面實測：handler 持 writer 鎖 12 秒（純 `BEGIN IMMEDIATE` + `ROLLBACK`），同時段三支探針 p50 3ms、>1s 次數 0。`engine.py:56` 設 `journal_mode=WAL` ⇒ reader 不被 writer 擋。 |
| **threadpool 飽和（中途提出）** | **自行撤回** | handler 自己推翻：`/api/crawler/jobs` 雖是 `async def`，但其相依 `get_worker`（`crawler_routes.py:22`）是 `sync def` ⇒ 依 `fastapi/dependencies/utils.py:673-676` 它**也**要抶 token。飽和實驗（RUN4/RUN6）中 `jobs` 確實退化至 204ms / 120s timeout，而歷史觀察是 **5ms** ⇒ 不符。 |
| **NFS I/O（原候選 2）** | **機制已消除** | 那組量測時 DB 在 NFS（`git show 73de0aa:docker-compose.yml:19`），`dec5b44` 已搬至本地 ext4。 |

### 並非一無所獲：本 BR 卡住的三輪挖出了一個**確定性**缺陷

追這張 BR 的 handler `ses_fdf8fc2c4ffeHyJI2iGo3sB5b8` 在設計負載探針時發現：
**`/api/search?q=the` 是確定性 92-95 秒**，單指令、零並發、不需負載。
→ 已另建 `BR-20260821_050000`（CRITICAL）並修復（`a31e925`，92s → 0.095s）。

**之前量不到是因為 idle 探針打的是 `/api/collections`，而慢的是 `/api/search` 的特定查詢。**
但注意：**那是另一張帳**——BR-050000 是確定性、本 BR 是偶發，
`jobs` 探針的形狀不同，不得宣稱 BR-050000 一併解釋了歷史 30.7s。

### 修復後的負載量測（附控制組）

```
40 併發 q=the（修復後，dispatcher 獨立重跑）
  health       n=8  med=0.002  max=0.011  over1s=0
  jobs         n=8  med=0.002  max=0.007  over1s=0
  collections  n=8  med=0.004  max=0.317  over1s=0
  LOAD        n=40  med=1.515  max=1.634  codes={'200'}
對照 RUN6（修復前、同樣 40 併發）：三支探針全部 120s timeout
⇒ 探針在退化存在時確實抓得到（有鑑別力），現在真的沒有
```

### 結案的代價（認了，不掩飾）

若尖峰日後復發，要從頭查 —— 沒有常設觀測留下連續訊號。
使用者在知道這個代價的前提下選了結案（另兩選項是「設常設觀測」與「人為製造 NFS 壓力」）。

**復發時的重開條件（寫給下一個人）**：若再量到 >2s 且形狀是
「`collections`/`search` 尖峰而 `jobs` 同刻正常」，**那是本 BR 的形狀**，重開它；
若三支同時尖峰，那是新的機制，開新張。兩者不同，不要共用一張帳。
